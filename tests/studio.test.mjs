...
test('serves real interface with CSP and a session token', async t => {
  ...
  assert.match(await res.text(), /Inspect a website/);
  for (const path of ['/styles.css', '/app.js', '/favicon.svg', '/interaction.css', '/interaction.js', '/page-grid.js']) assert.equal((await fetch(url + path)).status, 200);
});
