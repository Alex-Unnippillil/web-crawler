...
test('certificate failure is not retried', async () => { const x=mockCrawl({[seed]:()=>{throw new Error('CERT_HAS_EXPIRED');}},{retries:3}); const r=await x.result; assert.equal(x.calls.length,1); assert.equal(r.errors[0].kind,'tls'); });
