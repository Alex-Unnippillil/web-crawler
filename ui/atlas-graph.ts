...
export class Spiderweb {
  ...
  mount(target:HTMLElement):void {
    target.innerHTML=`<div class="atlas atlas-map"><div class="atlas-heading"><div><span class="atlas-kicker">SPIDERWEB EXPLORER</span><h3>Follow the connections.</h3><p>Observed links, grouped by URL path. Select a node to explore its neighborhood.</p></div><div class="atlas-actions"><button class="button" data-map="svg">Save SVG</button><button class="button" data-map="png">Save PNG</button><button class="button" data-map="json">Graph JSON</button></div></div>
      <div class="atlas-map-controls"><label class="atlas-search"><span>⌕</span><input id="map-search" type="search" placeholder="Find a URL or page title" aria-label="Search link map"></label><label>Layout<select id="map-layout"><option value="web">Spiderweb</option><option value="radial">Radial depth</option><option value="columns">Discovery layers</option></select></label><label>Path group<select id="map-group"><option value="">All paths</option></select></label><label>Node limit<select id="map-limit"><option>100</option><option selected>250</option><option>500</option></select></label></div>
  ...
  private draw(recalculate=true):void {
    ...
    root.querySelector('#map-limits')!.textContent=`Showing ${subset.nodes.length} / ${subset.matched} matching nodes; ${subset.edges.length} / ${subset.totalEdges} visible links. Amber: discovered. Coral: failed. Gray: external. Drag to move; scroll to zoom.`;
    root.querySelector('#map-legend')!.innerHTML=groups.filter(g=>subset.nodes.some(n=>n.group===g)).map(g=>`<span><svg width="9" height="9"><circle cx="4.5" cy="4.5" r="4" fill="${color(g==='external'?'external':'page',g)}"/></svg>${esc(g)}</span>`).join('');
    this.inspector();
  }
